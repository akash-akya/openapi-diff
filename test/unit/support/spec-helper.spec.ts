...
describe('spec-helper', () => {
    describe('OpenApiDiffError', () => {
        it('should find equal two OpenApiDiffError objects with same code and message', () => {
            const actual = new OpenApiDiffErrorImpl('OPENAPI_DIFF_FILE_SYSTEM_ERROR', 'A file system error');
            const expected = new OpenApiDiffErrorImpl('OPENAPI_DIFF_FILE_SYSTEM_ERROR', 'A file system error');
        ...
        it('should find not equal two OpenApiDiffError objects with different code', () => {
            const actual = new OpenApiDiffErrorImpl('OPENAPI_DIFF_SPEC_DESERIALISER_ERROR', 'the error message');
            const expected = new OpenApiDiffErrorImpl('OPENAPI_DIFF_FILE_SYSTEM_ERROR', 'the error message');

            expect(actual).not.toEqual(expected);
        });

        it('should find not equal two OpenApiDiffError objects with different message', () => {
            const actual = new OpenApiDiffErrorImpl('OPENAPI_DIFF_SPEC_DESERIALISER_ERROR', 'an error message');
            const expected = new OpenApiDiffErrorImpl(
                'OPENAPI_DIFF_SPEC_DESERIALISER_ERROR', 'a different error message'
            );
        ...
        it('should print OpenApiDiffErrors in a human readable format', () => {
            const error = new OpenApiDiffErrorImpl('OPENAPI_DIFF_SPEC_DESERIALISER_ERROR', 'An error occurred');

            expect(error.toString())
                .toBe('OpenApiDiffError: { code: OPENAPI_DIFF_SPEC_DESERIALISER_ERROR, message: An error occurred }');
        });

        it('should not break if the message contains sprintf escape sequences', () => {
            const error = new OpenApiDiffErrorImpl('OPENAPI_DIFF_FILE_SYSTEM_ERROR', 'A message with %some %percents');
